<nav x-data="{ open: false, masterOpen: false }" class="bg-gradient-to-r from-indigo-600 to-purple-600 shadow-lg hidden md:block">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-1 md:space-x-2 sm:-my-px sm:ms-4 md:ms-6 lg:ms-10 sm:flex items-center flex-wrap">
                    ...
                    @else
                    <a href="{{ route('report.kelas') }}" class="inline-flex items-center px-2 md:px-3 lg:px-4 py-2 text-xs md:text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('report.*') ? 'bg-white/20 text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                        <svg class="w-4 h-4 mr-1 md:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                        </svg>
                        <span class="hidden lg:inline">Report</span>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden md:hidden bg-white/10 backdrop-blur-lg">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @else
            <a href="{{ route('report.kelas') }}" class="block pl-3 pr-4 py-2 text-base font-medium {{ request()->routeIs('report.*') ? 'bg-white/20 text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' }} transition duration-150 ease-in-out">
                Report
            </a>
            @endif
        </div>
...
<nav class="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-indigo-600 to-purple-600 shadow-lg border-t border-white/20 md:hidden z-50">
    <div class="flex items-center justify-around h-16 px-2">
        ...
        @else
        <a href="{{ route('report.kelas') }}" class="flex flex-col items-center justify-center flex-1 py-2 px-2 transition-colors duration-200 {{ request()->routeIs('report.*') ? 'text-white bg-white/20 rounded-lg' : 'text-white/70 hover:text-white' }}">
            <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            <span class="text-xs font-medium">Report</span>
